...
package exponent.selfsignedcred_ble.opacity;
...
import exponent.selfsignedcred_ble.dhsdemo.Transceiver;
...
//        //logger.info(TAG,"Secure Message Signer:  "+smSignerCert.toString());
        ...
        logger.info(TAG, "Share Secret:  "+ByteUtil.toHexString(z, " "));
			...
			throws GeneralSecurityException
	{
		...
		} else
		{
            ...
            //    logger.error(TAG,"Proivders:  " + p.getName());
            //}
			KeyAgreement keyAgree = KeyAgreement.getInstance("ECDH");
			//logger.error(TAG, "KeyAgreement Provider: " + keyAgree.getProvider().getName());
            ...
            try
            {
                ...
                ks.load(null);
                Signature cvcSigner= Signature.getInstance("SHA256withECDSA");
                cvcSigner.initSign((PrivateKey) ks.getKey("selfSignedPivKey",null),new SecureRandom());
                cvcSigner.update(Cicc);
                cvcSig=cvcSigner.sign();
			...
			cardSignature = CardSignature.parse(sessionKeys.get("pub"));
			...
			throws GeneralSecurityException
	{
		...
		} else
		{
			...
			logger.newLine();
			logger.info(TAG, "Client Generated secp384r1 Ephemeral Pubic Key: " + ByteUtil.toHexString(clientPublicKey, " "));
            ...
            try
            {
                ks = KeyStore.getInstance("AndroidKeyStore");
                ks.load(null);
                Signature cvcSigner= Signature.getInstance("SHA384withECDSA");
                cvcSigner.initSign((PrivateKey) ks.getKey("selfSignedPivKey",null),new SecureRandom());
                cvcSigner.update(Cicc);
                cvcSig=cvcSigner.sign();
            } catch (KeyStoreException e)
			...
			cardSignature = CardSignature.parse192(sessionKeys.get("pub"));
